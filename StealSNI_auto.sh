...
if [ -z "$DOMAIN_NAME" ] || [ -z "$LETSENCRYPT_EMAIL" ]; then
    ...
    echo "Использование: $0 <DOMAIN> <EMAIL> [CF_API_TOKEN] [Z_KID] [Z_HMAC]" >&2
    exit 1
fi
...
ufw allow 22/tcp comment 'SSH' || { echo "❌ Ошибка при открытии端口 SSH"; exit 1; }
...
# --- Получение SSL ---
echo ""
echo "--- Этап 7: Получение SSL-сертификата ---"

LE_SUCCESS=0

if [ -n "$CF_API_TOKEN" ]; then
    echo "🔑 Автоматический режим: Пробуем Let's Encrypt через Cloudflare API..."
    ...
    if certbot certonly \
        ...
        -d "$DOMAIN_NAME" \
        --non-interactive --agree-tos --email "$LETSENCRYPT_EMAIL"; then
        LE_SUCCESS=1
    fi
    rm -f "$CF_CREDS_FILE"
else
    echo "⚠️ Ручной режим: Токен CF не передан. Требуется ручной ввод для Let's Encrypt!"
    if certbot certonly --manual --preferred-challenges dns -d "$DOMAIN_NAME" --agree-tos --email "$LETSENCRYPT_EMAIL"; then
        LE_SUCCESS=1
    fi
fi

# Фоллбек на ZeroSSL, если Let's Encrypt выдал ошибку
if [ "$LE_SUCCESS" -ne 1 ]; then
    echo "⚠️ [WARN] Let's Encrypt вернул ошибку (возможно, достигнуты лимиты)."
    
    if [ -n "$Z_KID" ] && [ -n "$Z_HMAC" ]; then
        echo "🔄 Переключаемся на резервный CA: ZeroSSL..."
        
        # Регистрируем аккаунт с EAB ключами в Certbot
        certbot register \
        ...
        # Запрашиваем сертификат у ZeroSSL
        if [ -n "$CF_API_TOKEN" ]; then
            mkdir -p /root/.secrets/certbot
            CF_CREDS_FILE="/root/.secrets/certbot/cloudflare.ini"
            printf "dns_cloudflare_api_token = %s\n" "$CF_API_TOKEN" > "$CF_CREDS_FILE"
        ...
        if [ "$Z_STATUS" -ne 0 ]; then
            echo "❌ Ошибка: Не удалось выпустить сертификат ни через Let's Encrypt, ни через ZeroSSL!" >&2
            exit 1
        fi
    else
        echo "❌ Ошибка: Выпуск Let's Encrypt прерван, а ключи ZeroSSL (Z_KID/Z_HMAC) не переданы для фоллбека!" >&2
        exit 1
    fi
fi
